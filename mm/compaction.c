...
static unsigned long __pageblock_count_order(struct zone *zone,
		int mtype, int order)
{
...
{
	return (__pageblock_count_order(zone, order, mtype)
			>= nr_expect);
}
...
#ifdef CONFIG_COMPACTION_SPECIFIED
	cc.order = order;
	cc.force_compact = true;
	cc.nr_expect = nrpages;
#endif
	for (zoneid = 0; zoneid < MAX_NR_ZONES; zoneid++) {
