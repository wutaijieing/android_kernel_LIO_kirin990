                                   ...
                                   struct snd_pcm_hw_params *params)
{
    ...
    if (ret < 0) {
        pr_err("hw params: set DAI configuration failed %d\n", ret);
        return ret;
    }
    ...
    if (ret < 0) {
        pr_err("hw params: set system clock failed %d\n", ret);
        return ret;
    }
...
#endif
    if (NULL == pdev) {
        pr_err("probe: enter with null pointer, fail!\n");
        return -EINVAL;
    }
...
static int __init hi6xxx_da_separate_soc_init(void)
{
    logi("soc init\n");
    return platform_driver_register(&hi6250_hi6555c_driver);
}

static void __exit hi6xxx_da_separate_soc_exit(void)
{
    logi("soc exit\n");
    platform_driver_unregister(&hi6250_hi6555c_driver);
}

module_init(hi6xxx_da_separate_soc_init);
module_exit(hi6xxx_da_separate_soc_exit);
