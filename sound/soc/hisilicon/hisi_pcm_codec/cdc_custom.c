...
static struct snd_soc_dai_driver hisi_pcm_codec_dai[] = {
	{
		.name = "hi6210-mm",
		.playback = {
...
static const struct snd_pcm_hardware *_get_pcm_hw_config(int device, int stream)
{
	if (device < 0 || device >= PCM_DEVICE_TOTAL ||
		stream < 0 || stream >= PCM_STREAM_MAX)
		return NULL;
...
static void _capture_data_process(struct data_convert_info *info, int device)
{
	...
	unsigned int period_bytes = info->period_size * sample_bytes * channels;

	if (channels == RECORD_CHANNEL_CNT_MAX && device == 0)
		memcpy(info->dest_cfg.addr, info->src_cfg.addr, period_bytes);
}
...
static void _init_cust_interface(struct cust_priv *priv_data)
{
	...
	priv_data->pcm_dai = hisi_pcm_codec_dai;
	priv_data->dai_num = ARRAY_SIZE(hisi_pcm_codec_dai);
...
static int cdc_custom_probe(struct platform_device *pdev)
{
	...
	hisi_pcm_codec_register_cust_interface(priv_data);
