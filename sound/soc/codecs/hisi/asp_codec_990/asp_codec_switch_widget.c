...
static int vo_up_l_48k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int vo_up_r_48k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int mic3_48k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int mic4_48k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int s3_48k_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int audio_2mic_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = asp_codec_get_codec();
	struct asp_codec_priv *priv = snd_soc_codec_get_drvdata(codec);
...
static int voice_2mic_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int mic34_bt_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int mdm5g_up_l_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		asp_codec_clr_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << MDM_5G_L_UP_AFIFO_CLKEN_OFFSET);
		udelay(CLEAR_FIFO_DELAY_LEN_MS);
		asp_codec_set_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << MDM_5G_L_UP_AFIFO_CLKEN_OFFSET);
		break;
...
static int mdm5g_up_r_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		asp_codec_clr_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << MDM_5G_R_UP_AFIFO_CLKEN_OFFSET);
		udelay(CLEAR_FIFO_DELAY_LEN_MS);
		asp_codec_set_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << MDM_5G_R_UP_AFIFO_CLKEN_OFFSET);
		break;
...
static int audio_dn_l_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int audio_dn_r_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int codec3_dn_l_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
...
static int ultr_dn_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		asp_codec_clr_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << ULTR_L_DN_AFIFO_CLKEN_OFFSET);
		asp_codec_clr_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << ULTR_R_DN_AFIFO_CLKEN_OFFSET);
		udelay(CLEAR_FIFO_DELAY_LEN_MS);
		asp_codec_set_reg_bits(CODEC_CLK_EN0_REG,
			0x1 << ULTR_L_DN_AFIFO_CLKEN_OFFSET);
		asp_codec_set_reg_bits(CODEC_CLK_EN0_REG,
		...
		/* ultr fifo fs 96k */
		asp_codec_reg_update(FS_CTRL0_REG,
			MAX_VAL_ON_BIT(2) << FS_ULTR_L_DN_AFIFO_OFFSET,
			0x1 << FS_ULTR_L_DN_AFIFO_OFFSET);
		asp_codec_reg_update(FS_CTRL2_REG,
			MAX_VAL_ON_BIT(2) << FS_ULTR_R_DN_AFIFO_OFFSET,
...
/* SWITCH - MDM5G */
static const struct snd_kcontrol_new dapm_mdm5g_up_l_o_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR0_REG_ADDR, MDM5G_UP_L_O_BIT, 1, 0);
static const struct snd_kcontrol_new dapm_mdm5g_up_r_o_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR0_REG_ADDR, MDM5G_UP_R_O_BIT, 1, 0);
/* SWITCH - ULTR_UP */
...
static const struct snd_kcontrol_new dapm_ultr_dn_switch_controls =
	SOC_DAPM_SINGLE("ENABLE", CODEC_VIR1_REG_ADDR, ULTR_DN_BIT, 1, 0);
...
#define SWITCH_WIDGET \
	...
	SND_SOC_DAPM_SWITCH_E("VOICE_4MIC_O_SWITCH", SND_SOC_NOPM, 0, 0, \
		...
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
	SND_SOC_DAPM_SWITCH_E("MDM5G_UP_L_O_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_mdm5g_up_r_o_switch_controls, mdm5g_up_l_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
	SND_SOC_DAPM_SWITCH_E("MDM5G_UP_R_O_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_mdm5g_up_l_o_switch_controls, mdm5g_up_r_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)), \
	...
	SND_SOC_DAPM_SWITCH_E("ULTR_DN_SWITCH", SND_SOC_NOPM, 0, 0, \
		&dapm_ultr_dn_switch_controls, ultr_dn_power_event, \
		(SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD)) \
