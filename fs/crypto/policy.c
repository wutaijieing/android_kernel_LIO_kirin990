				...
				const struct fscrypt_policy *policy)
{
	...
	/* get nonce and iv */
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	generate_nonce(nonce, inode, FS_KEY_DERIVATION_NONCE_SIZE);
#else
						...
						void *fs_data, bool preload)
{
	...
	/* get nonce and iv */
#ifdef CONFIG_SCSI_UFS_ENHANCED_INLINE_CRYPTO_V3
	generate_nonce(nonce, child, FS_KEY_DERIVATION_NONCE_SIZE);
#else
	get_random_bytes(nonce, FS_KEY_DERIVATION_NONCE_SIZE);
#endif
	get_random_bytes(ctx.iv, FS_KEY_DERIVATION_IV_SIZE);
...
#if (defined CONFIG_HWAA) && (!defined CONFIG_DISABLE_HWAA)
