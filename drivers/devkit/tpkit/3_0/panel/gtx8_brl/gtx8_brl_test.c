...
static int brl_init_testlimits(struct brl_ts_test *ts_test)
{
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_UNIFIED_LIMIT,
		data_buf, 1, 3);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_UNIFIED_LIMIT);
		return ret;
	}
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_NOISE_LIMIT,
		data_buf, 1, 1);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_NOISE_LIMIT);
		return ret;
	}
	test_params->noise_threshold = (s16)data_buf[0];

	/* <self max_threshold, min_threshold> */
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELF_UNIFIED_LIMIT,
		data_buf, 1, 2);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SELF_UNIFIED_LIMIT);
		return ret;
	}
	/* store data to test_parms */
	for (i = 0; i < MAX_DRV_NUM + MAX_SEN_NUM; i++) {
	...
	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SELFNOISE_LIMIT,
		data_buf, 1, 1);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SELFNOISE_LIMIT);
		return ret;
	}
	test_params->self_noise_threshold = data_buf[0];

	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SPECIAL_SELFRAW_MAX,
		(int32_t *)test_params->self_max_limits,
			1, test_params->drv_num + test_params->sen_num);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SPECIAL_SELFRAW_MAX);
		ret = 0;
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SPECIAL_SELFRAW_MIN,
		(int32_t *)test_params->self_min_limits,
			1, test_params->drv_num + test_params->sen_num);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SPECIAL_SELFRAW_MIN);
		ret = 0;
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SPECIAL_RAW_MAX,
		(int32_t *)test_params->max_limits,
		1, test_params->drv_num * test_params->sen_num);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SPECIAL_RAW_MAX);
		ret = 0;
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SPECIAL_RAW_MIN,
		(int32_t *)test_params->min_limits,
		1, test_params->drv_num * test_params->sen_num);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SPECIAL_RAW_MIN);
		ret = 0;
	}

	ret = ts_kit_parse_csvfile(file_path, CSV_TP_SPECIAL_RAW_DELTA,
		data_buf, 1, 1);
	if (ret) {
		TS_LOG_INFO("Failed get %s", CSV_TP_SPECIAL_RAW_DELTA);
		ret = 0;
	} else {
		for (i = 0; i < MAX_DRV_NUM * MAX_SEN_NUM; i++)
			test_params->deviation_limits[i] = data_buf[0];
	}
	return ret;
}
...
static int analysis_delta_data(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.deviation_limits[0]) {
			TS_LOG_ERR("the tx_data is not in range, value:%d threshold:%d",
				val, ts_test->test_params.deviation_limits[0]);
			ret = -1;
			break;
		}
	}
	...
	for (i = 0; i < size; i++) {
		...
		if (val > ts_test->test_params.deviation_limits[0]) {
			TS_LOG_ERR("the rx_data is not in range, value:%d threshold:%d",
				val, ts_test->test_params.deviation_limits[0]);
			ret = -1;
			break;
		}
...
cache_exit:
	...
	buf[0] = 0x00;
	ts_test->ts->ops.i2c_write(flag_addr, &buf[0], 1);
	return ret;
}
...
static int brl_cache_diffdata(struct brl_ts_test *ts_test)
{
	...
	if (data_size > sen_num * drv_num || data_size <= 0) {
		TS_LOG_ERR("Invalid rawdata size(%u)", data_size);
		return ret;
	}

	/* get diffdata */
	data_addr = ts_test->test_params.diffdata_addr;
	for (j = 0; j < MAX_DIFFDATA_TIMES; j++) {
		while (retry--) {
			ret = ts_test->ts->ops.i2c_read(flag_addr, &buf[0], 1);
			if ((ret == 0) && (buf[0] & 0x80) == 0x80) {
				TS_LOG_INFO("diffdata %d is ready", j);
				break;
			}
			msleep(20); /* 20 : 20ms */
		}
		...
		if (retry < 0) {
			TS_LOG_ERR("diffdata %d is not ready", j);
			ret = -EINVAL;
			goto cache_exit;
		}
		ret = ts_test->ts->ops.i2c_read(data_addr,
				(u8 *)&ts_test->noisedata[j].data[0],
				data_size * sizeof(u16));
		if (ret) {
			TS_LOG_ERR("Failed to read noisedata:%d", ret);
			goto cache_exit;
		}
		for (i = 0; i < data_size; i++) {
			if (ts_test->noisedata[j].data[i] < 0)
				ts_test->noisedata[j].data[i] =
					-ts_test->noisedata[j].data[i];
		}
		ts_test->noisedata[j].size = data_size;
	}
...
static u8 analysis_raw_data(struct brl_ts_test *ts_test)
{
	...
	for (i = 0; i < size; i++) {
		...
		if ((val < ts_test->test_params.min_limits[i]) ||
			...
			TS_LOG_ERR("the mutual raw data is not in range, value:%d threshold:%d %d",
	...
	for (i = 0; i < size; i++) {
		...
		if ((val < ts_test->test_params.self_min_limits[i]) ||
			...
			TS_LOG_ERR("the self raw data is not in range, value:%d threshold:%d %d",
...
static u8 analysis_diff_data(struct brl_ts_test *ts_test)
{
	u32 i;
	u32 size;
	u8 ret = 0x00;
	...
	struct ts_rawdata_info_new *info, struct brl_ts_test *ts_test)
{
	...
	int rotate_type = GT_RAWDATA_CSV_RXTX_ROTATE;
	...
	pts_node->size = ts_test->tx_delta.size + ts_test->rx_delta.size;
	if (pts_node->size) {
	...
	if (ts_test->noisedata[0].size) {
		pts_node->values = kcalloc(ts_test->noisedata[0].size,
			sizeof(int), GFP_KERNEL);
		if (!pts_node->values) {
			TS_LOG_ERR("malloc failed  for values\n");
			kfree(pts_node);
		...
		for (i = 0; i < ts_test->noisedata[0].size; i++)
			pts_node->values[i] = ts_test->noisedata[0].data[i];
		ts_kit_rotate_rawdata_abcd2cbad(info->rx, info->tx,
			pts_node->values, rotate_type);
		/* calculate rawdata min avg max vale */
	...
	pts_node->size = ts_test->noisedata[0].size;
	...
	snprintf(fw_vendor, sizeof(fw_vendor),
		"%02X_%02X_%u", ts->hw_info.patch_vid[PATCH_VID_MINOR],
