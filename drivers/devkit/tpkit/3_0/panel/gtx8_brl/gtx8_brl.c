...
#define GOODIX_REG_CLK_GATE 0xD806
#define GOODIX_CLK_GATE_OPEN 0x77
#define GOODIX_REG_TRIM 0xD006
#define GOODIX_TRIM_D12 0x3C
#define GOODIX_REG_RESET 0xD808
#define GOODIX_RESET_EN 0xFA
static void brl_reset_later(void)
{
	u8 val;

	usleep_range(USLEEP_START, USLEEP_END);
	val = GOODIX_CLK_GATE_OPEN;
	brl_i2c_write(GOODIX_REG_CLK_GATE, &val, VAL_LEN);
	val = GOODIX_TRIM_D12;
	brl_i2c_write(GOODIX_REG_TRIM, &val, VAL_LEN);
	usleep_range(USLEEP_START, USLEEP_END);
	val = GOODIX_RESET_EN;
	brl_i2c_write(GOODIX_REG_RESET, &val, VAL_LEN);
}
...
static int brl_chip_detect(struct ts_kit_platform_data *pdata)
{
	...
	ret = brl_power_init();
	if (ret < 0) {
		TS_LOG_ERR("brl_power_init error %d", ret);
		goto out;
	}
	...
	if (ret < 0) {
		TS_LOG_ERR("failed to enable power, ret = %d", ret);
	...
	while (retry) {
		ret = brl_chip_reset();
		if (ret < 0)
			TS_LOG_ERR("failed to chip_reset, ret = %d", ret);
