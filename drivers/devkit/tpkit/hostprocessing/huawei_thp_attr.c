	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (multi_screen_status_switch(screen_status))
		thp_log_err("multi_screen_status_switch fail\n");
#endif
	return count;
}
...
static DEVICE_ATTR(roi_data_internal, 0444, thp_roi_data_debug_show, NULL);
