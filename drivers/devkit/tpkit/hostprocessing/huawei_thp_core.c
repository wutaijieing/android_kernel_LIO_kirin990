...
static struct thp_queue *thp_queue_init(void)
{
	...
	queue->frame_data = vzalloc(THP_LIST_MAX_FRAMES * sizeof(*queue->frame_data));
	if (queue->frame_data == NULL) {
		thp_log_err("%s frame_data malloc failed\n", __func__);
		kfree(queue);
		queue = NULL;
		return NULL;
	}
...
static bool thp_queue_dequeue(struct thp_queue *queue)
{
	...
	memset(&queue->frame_data[queue->front], 0, sizeof(*queue->frame_data));
	...
	u8 *read_buf, int len)
{
	...
	if (len > THP_QUEUE_NODE_BUFF_MAX_LEN) {
		thp_log_debug("%s len is too big\n", __func__);
		len = THP_QUEUE_NODE_BUFF_MAX_LEN;
	}
...
static void thp_queue_clear_all(struct thp_queue *queue)
{
	...
	memset(queue->frame_data, 0, sizeof(*queue->frame_data));
	queue->front = 0;
	queue->tail = 0;
	queue->size = THP_LIST_MAX_FRAMES;
	queue->flag = QUEUE_IS_NOT_FULL;
...
static void thp_queue_free(struct thp_queue *queue)
{
	...
	vfree(queue->frame_data);
	queue->frame_data = NULL;
	kfree(queue);
	queue = NULL;
}
...
int multi_screen_status_switch(u32 current_status)
{
	...
	/* status switch unfold to fold */
	if (current_status == SCREEN_FOLDED) {
		if (cd->ts_suspended && cd->suspended) {
			thp_multi_power_control_notify(TS_SCREEN_SWITCH,
			...
			for (i = 0; i < PM_TYPE_LIST_LEN; i++) {
				...
				thp_multi_power_control_notify(pm_type_list[i],
					NO_SYNC_TIMEOUT, SUB_TOUCH_PANEL);
				thp_multi_power_control_notify(pm_type_list[i],
					NO_SYNC_TIMEOUT, MAIN_TOUCH_PANEL);
			}
		}
	...
	if (current_status == SCREEN_UNFOLD) {
		thp_multi_power_control_notify(TS_SCREEN_SWITCH,
			NO_SYNC_TIMEOUT, SUB_TOUCH_PANEL);
		thp_log_info("%s: switch to SCREEN_UNFOLDED\n", __func__);
	}
	return 0;
}
	...
	int panel_index)
{
	...
	if (cd->multi_panel_index == SINGLE_TOUCH_PANEL)
		return thp_power_control_notify(pm_type, timeout);
	...
	case SUB_TOUCH_PANEL:
		...
		return ts_kit_multi_power_control_notify(pm_type, timeout,
			panel_index);
#endif
	default:
	...
	struct thp_ioctl_get_frame_data *data)
{
	...
	if (data->size > THP_QUEUE_NODE_BUFF_MAX_LEN) {
		thp_log_debug("%s data size too big, set to 8000\n",
			__func__);
		data->size = THP_QUEUE_NODE_BUFF_MAX_LEN;
	}
...
static int thp_probe(struct spi_device *sdev)
{
	...
	if (thp_core->use_thp_queue) {
		thp_core->thp_queue = thp_queue_init();
