		...
		struct dubai_thread_entry **thread)
{
	...
	if (task->flags & PF_KTHREAD) {
		tgid = KERNEL_TGID;
		strncpy(comm, KERNEL_NAME, TASK_COMM_LEN - 1);
	} else {
		if (leader != NULL)
			strncpy(comm, leader->comm, TASK_COMM_LEN - 1);
		else
...
int dubai_update_proc_cputime(void)
{
	...
	do_each_thread(temp, task) {
		...
		if (entry == NULL) {
			...
			dubai_err("Failed to find the entry for process %d", task->tgid);
			return -ENOMEM;
		}
...
static void dubai_get_active_process(struct buffered_log_entry *log_entry)
{
	...
	hash_for_each(proc_hash_table, bkt, entry, hash) {
		...
		if (!dubai_check_update_entry_state(entry, DUBAI_STATE_ALIVE, DUBAI_STATE_DEAD) ||
		...
		list_for_each_entry(thread, &entry->threads, node) {
			...
			if (!dubai_check_update_entry_state(thread, DUBAI_STATE_ALIVE, DUBAI_STATE_DEAD) ||
				((thread->utime + thread->stime) == 0))
				continue;
	...
	unsigned long __always_unused cmd, void *v)
{
	...
	if (task->tgid == task->pid || dubai_task_alive(task->group_leader)) {
		...
		entry = dubai_find_proc_entry(task->pid, tgid, task->comm, &thread);
	}
	if (entry == NULL)
		goto exit;
