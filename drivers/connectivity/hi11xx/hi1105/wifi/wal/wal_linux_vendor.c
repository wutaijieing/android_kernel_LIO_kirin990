    ...
    uint8_t *sniffer_ta_mac, uint8_t sniffer_mode, uint8_t *cap_channel, uint8_t *cap_protocol_mode)
{
    ...
    wal_netdev_stop(p_net_dev); // sniffer结束，wlan0 down
    return OAL_SUCC;
}
...
int32_t wal_hid2d_sleep_mode(oal_net_device_stru *pst_net_dev, uint8_t uc_ctrl)
{
    char uc_cmd[BUFF_SIZE];

    oam_warning_log1(0, OAM_SF_ANY, "wal_hid2d_sleep_mode::ctrl = %d", uc_ctrl);
    if (snprintf_s(uc_cmd, BUFF_SIZE, BUFF_SIZE - 1, "%s%d", uc_cmd, uc_ctrl) > 0) {
        return wal_hipriv_sta_pm_on(pst_net_dev, (uint8_t*) uc_cmd);
    }
    return -OAL_EFAUL;
}
...
OAL_STATIC int32_t wal_ioctl_set_hid2d_state(oal_net_device_stru *pst_net_dev, uint8_t uc_param)
{
    ...
    if (uc_param == ENABLE) {
        ...
        wal_hid2d_sleep_mode(pst_net_dev, DISABLE);
        ...
        wal_hid2d_sleep_mode(pst_net_dev, ENABLE);
    ...
    uint8_t *pc_cmd)
{
    if (oal_strncasecmp(pc_cmd, CMD_PCIE_ASPM_STATE, CMD_PCIE_ASPM_STATE_LEN) == 0) {
        ...
        wal_ioctl_set_hid2d_state(pst_net_dev, ENABLE);
    } else if  (oal_strncasecmp(pc_cmd, CMD_LOW_LATENCY_OFF, CMD_LOW_LATENCY_OFF_LEN) == 0) {
        wal_ioctl_set_hid2d_state(pst_net_dev, DISABLE);
    }
#endif
#ifdef _PRE_WLAN_FEATURE_HID2D_PRESENTATION
    else {
    ...
    oal_ifreq_stru *p_ifr, wal_wifi_priv_cmd_stru *p_priv_cmd, uint8_t *p_cmd)
{
    ...
    return wal_vendor_priv_cmd_hid2d(p_net_dev, p_ifr, p_cmd);
}
