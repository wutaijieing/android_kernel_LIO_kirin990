...
{
#define SMBIOS_WLAN_ADDR 0x10B02049
#define SMBIOS_PAGE_SIZE 0x1000
#define WLAN_INSMOD_DISABLE 1
    int32_t ret;
    char wflag; /* 1.disable 其他: enable */
    void *smbios_vir_wlan = NULL;

    smbios_vir_wlan = memremap(SMBIOS_WLAN_ADDR, SMBIOS_PAGE_SIZE, MEMREMAP_WB);
    if (!smbios_vir_wlan) {
        oal_io_print("is_hisi_insmod_hi110x_wlan memremap fail.\n");
        return -OAL_EFAIL;
    }

    wflag = *(char*)smbios_vir_wlan;
    if (wflag == WLAN_INSMOD_DISABLE) {
        oal_io_print("is_hisi_insmod_hi110x_wlan:needn't insmod ko.\n");
        ret = -OAL_EFAIL;
    } else {
