...
uint32_t mac_smps_update_user_status(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user,
    oal_bool_enum_uint8 en_from_beacon)
{
    ...
    if ((pst_mac_vap == NULL) || (pst_mac_user == NULL)) {
    ...
    switch (en_user_smps_mode) {
        ...
        default:
            oam_warning_log1(pst_mac_vap->uc_vap_id, OAM_SF_SMPS,
                "{mac_smps_update_user_status: en_user_smps_mode mode[%d] fail!}", en_user_smps_mode);
            return OAL_FAIL;
    }
    ...
    if (g_wlan_spec_cfg->feature_11ax_is_open) {
        if (en_from_beacon && pst_mac_user->bit_have_recv_he_rom_flag &&
            ...
            oam_warning_log2(pst_mac_vap->uc_vap_id, OAM_SF_SMPS,
                "{mac_smps_update_user_status:en_avail_num_spatial_stream=[%d], en_he_rom_nss=[%d]!}",
