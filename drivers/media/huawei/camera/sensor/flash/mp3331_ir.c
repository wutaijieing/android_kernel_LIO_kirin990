...
enum mp3331_ito_threshold_param {
	ITO_THR_MIN = 0,
	ITO_THR_MAX = 1,
	ITO_THR_MAX_INDEX = 2,
};
...
static struct hw_mp3331_ir_private_data_t g_mp3331_ir_pdata;
static struct hw_flash_ctrl_t g_mp3331_ir_ctrl;
static struct i2c_driver g_mp3331_ir_i2c_driver;
...
static int hw_mp3331_ir_clear_err_and_unlock(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	if (fault_h & (MP3331_IR_OVER_VOLTAGE_PROTECT | MP3331_IR_VOUT_SHORT |
		MP3331_IR_LED_SHORT)) {
		if (!dsm_client_ocuppy(client_flash)) {
			dsm_client_record(client_flash,
...
static int hw_mp3331_ir_flash_mode(struct hw_flash_ctrl_t *flash_ctrl, int data)
{
	...
	if (pdata->need_flash_ldo == 1) {
		loge_if_ret(i2c_func->i2c_write(i2c_client,
			REG_PEAK_CUR_SET, FS_SD) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_LOW_VLO_SET,
			VBL_RUN_DIS) < 0); // diable low voltage
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_INDICATOR_SET,
			LED_OTAD | TUP_I | VTH_PAS) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client,
	...
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_LED_FLASH_CUR_SET,
		current_level) < 0);
	loge_if_ret(i2c_func->i2c_write(i2c_client,
		REG_FLASH_TIMER_SET, FL_TIM) < 0);
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_MODE_SET,
		STB_LV | FLASH_MODE_CUR | LED1_EN) < 0);
...
static int hw_mp3331_ir_torch_mode(struct hw_flash_ctrl_t *flash_ctrl, int data)
{
	...
	if (pdata->need_flash_ldo == 1) {
		...
		loge_if_ret(i2c_func->i2c_write(i2c_client,
			REG_TXMASK_CUR_SET, I_TX) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_MODE_SET,
			STB_LV | TORCH_TX_MODE_CUR | TORCH_PIN) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client,
			REG_LED_TORCH_CUR_SET,
			current_level) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_MODE_SET,
			STB_LV | TORCH_TX_MODE_CUR | TORCH_PIN | LED1_EN) < 0);
	} else {
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_MODE_SET,
			STB_LV | TORCH_MODE_CUR) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client,
			REG_LED_TORCH_CUR_SET, current_level) < 0);
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_MODE_SET,
	...
	if (pdata->ito_enable) {
		...
		if (dsm_flag) {
			...
			if (!dsm_client_ocuppy(client_flash)) {
				...
				cam_warn("[I/DSM] %s report[%d]", __func__,
					DSM_ITO_OUT_OF_RANGE_ERROR_NO);
			}
			return -EINVAL;
		}
...
static int hw_mp3331_ir_get_ito_protect_data(struct device_node *of_node,
		...
		if (rc < 0) {
			...
			for (i = 0; i < ITO_THR_MAX_INDEX; i++)
				cam_info("%s ito_protect_threshold[%d] = %d\n",
					__func__, i, pdata->ito_threshold[i]);
		}
	}
...
static int hw_mp3331_ir_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	rc = hw_mp3331_ir_get_ito_protect_data(of_node, pdata);
	return rc;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = scnprintf(buf, PAGE_SIZE, "%d %d %d %d",
		g_mp3331_ir_pdata.selftest_param[IR_SELFTEST_ON_TIME_INDEX],
		g_mp3331_ir_pdata.selftest_param[IR_SELFTEST_OFF_TIME_INDEX],
		g_mp3331_ir_pdata.selftest_param[IR_SELFTEST_CURRENT_INDEX],
		g_mp3331_ir_pdata.selftest_param[IR_SELFTEST_TIMES_INDEX]);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rc = hw_mp3331_ir_param_check((char *)buf, param,
	...
	// 0~1500 mA
	if ((cdata.data < 0) || (cdata.data > MP3331_FLASH_MAX_CUR)) {
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_mp3331_ir_off(&g_mp3331_ir_ctrl);
		if (rc < 0) {
		...
		cdata.data =
			g_mp3331_ir_pdata.ecurrent[CURRENT_TORCH_LEVEL_RT];
		cam_info("%s mode = %d, current = %d",
			__func__, cdata.mode, cdata.data);

		rc = hw_mp3331_ir_on(&g_mp3331_ir_ctrl, &cdata);
		if (rc < 0) {
	...
	enum led_brightness brightness)
{
	...
	if (led_bright == STANDBY_MODE) {
		...
		cdata.mode = TORCH_MODE;
		cdata.data =
			g_mp3331_ir_pdata.ecurrent[CURRENT_TORCH_LEVEL_MMI];
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static struct device_attribute g_mp3331_ir_lightness =
	__ATTR(flash_lightness_f,
	0660, hw_mp3331_ir_lightness_show, hw_mp3331_ir_lightness_store);
#endif

static int hw_mp3331_ir_register_attribute(struct hw_flash_ctrl_t *flash_ctrl,
	struct device *dev)
{
	int rc;
	...
	flash_ctrl->cdev_torch.brightness_set =
		hw_mp3331_ir_torch_brightness_set;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	rc = device_create_file(dev, &g_mp3331_ir_lightness);
	if (rc < 0) {
...
static int hw_mp3331_ir_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	if ((pdata->need_flash_ldo == 1) && (pdata->is_power_on == 0)) {
		loge_if_ret(aux_ldo_supply_enable(flash_ctrl,
			pdata->flash_ldo_voltage) < 0);
		pdata->is_power_on = 1;
	}
	...
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_INDICATOR_SET,
		LED_OTAD | TUP_I | VTH_PAS | INDI_PWM) < 0);
	loge_if_ret(i2c_func->i2c_write(i2c_client,
	...
	rc = register_camerafs_attr(&g_mp3331_ir_lightness);
	if (rc < 0) {
		cam_err("%s failed to creat lightness attribute", __func__);
		return -EINVAL;
	}
...
static int hw_mp3331_ir_remove(struct i2c_client *client)
{
	...
	g_mp3331_ir_ctrl.func_tbl->flash_exit(&g_mp3331_ir_ctrl);
...
static const struct i2c_device_id g_mp3331_ir_id[] = {
	{ "mp3331_ir", (unsigned long) &g_mp3331_ir_ctrl },
	{}
};

static const struct of_device_id g_mp3331_ir_dt_match[] = {
...
static struct i2c_driver g_mp3331_ir_i2c_driver = {
	...
	.id_table = g_mp3331_ir_id,
	.driver = {
		.name = "hw_mp3331_ir",
		.of_match_table = g_mp3331_ir_dt_match,
	},
};
...
{
	cam_info("%s enter\n", __func__);
	return i2c_add_driver(&g_mp3331_ir_i2c_driver);
}

static void __exit hw_mp3331_ir_module_exit(void)
{
	cam_info("%s enter", __func__);
	i2c_del_driver(&g_mp3331_ir_i2c_driver);
}

static struct hw_flash_i2c_client g_mp3331_ir_i2c_client;

static struct hw_flash_fn_t g_mp3331_ir_func_tbl = {
...
static struct hw_flash_ctrl_t g_mp3331_ir_ctrl = {
	.flash_i2c_client = &g_mp3331_ir_i2c_client,
	.func_tbl = &g_mp3331_ir_func_tbl,
	.hw_flash_mutex = &flash_mut_mp3331_ir,
	.pdata = (void *)&g_mp3331_ir_pdata,
