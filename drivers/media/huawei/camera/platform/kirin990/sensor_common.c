...
static struct mutex g_multiplex_gpio_lock;
...
static int g_boost_ctrl_vote; /* 5V boost vote */
...
int mclk_config(sensor_t *s_ctrl, unsigned int id, unsigned int clk, int on)
{
	...
	bool faocamera = (id == HWSENSOR_POSITION_SENSORHUB);
	...
	if (fsnclk0) {
		...
		if (IS_ERR_OR_NULL(s_ctrl->ao_camera)) {
			dev_err(dev, "could not get c_ao_camera\n");
	...
	int state)
{
	...
	if (pin_type == FSIN) {
		...
		if (g_multiplex_gpio_count > 0 &&
			index >= 0 && index < MULTIPLEX_GPIO_MAX) {
			mutex_lock(&g_multiplex_gpio_lock);
			if (state == POWER_OFF) {
				if (g_state_votes[index] > 1) {
					cam_info("g_state_votes = %d, cannot powerdown",
						g_state_votes[index]);
					g_state_votes[index]--;
					...
					if (rc < 0)
						...
						cam_info("succ config gpio%d output %d",
							sensor_info->gpios[pin_type].gpio,
							output);
					g_state_votes[index] = 0;
				} else {
					cam_err("%s invalid vote count = %d",
						__func__,
						g_state_votes[index]);
					rc = -1;
				}
			...
			if (state == POWER_ON) {
				if (g_state_votes[index] > 0) {
					cam_info("g_state_votes = %d, need not to powerup",
						g_state_votes[index]);
					g_state_votes[index]++;
					...
					cam_err("%s invalid vote count = %d",
						__func__, g_state_votes[index]);
					rc = -1;
				}
			}
			mutex_unlock(&g_multiplex_gpio_lock);
		} else {
			rc = gpio_direction_output(
				sensor_info->gpios[pin_type].gpio, output);
			if (rc < 0) {
	...
	struct sensor_power_setting *power_setting, int on_off)
{
	...
	case SENSOR_RST:
		...
		if (on_off == POWER_ON) {
			...
			if (rc > 0) {
				g_reset = rc;
				cam_err("%s btb is break %u", __func__, g_reset);
			}
	...
	case SENSOR_BOOT_5V:
		...
		if (on_off == POWER_ON) {
			...
			if (g_boost_ctrl_vote > 0)
				...
				cam_err("%s invalid g_boost_ctrl_vote = %d",
					__func__, g_boost_ctrl_vote);
		}
		break;
	default:
	...
	struct device_node *of_node, hwsensor_board_info_t *sensor_info)
{
	...
	ret = of_property_read_u32(of_node, "huawei,phyclk",
		&sensor_info->phy_clk_freq);
	if (ret) {
	...
	hwsensor_board_info_t *sensor_info, int info_count)
{
	...
	ret = (int)((unsigned int)ret |
		(unsigned int)of_property_read_u32_array(of_node,
			"huawei,phy_id",
			(unsigned int *)sensor_info->phyinfo.phy_id,
			count));
	...
	sensor_t *sensor)
{
	...
	if (count > 0) {
		ret = of_property_read_u32_array(of_node, "huawei,csi_index",
			(unsigned int *)&sensor_info->csi_id,
			count);
	} else {
	...
	/* get ldo */
	sensor_info->ldo_num =
		of_property_count_strings(of_node, "huawei,ldo-names");
	...
	for (i = 0; (int)i < sensor_info->gpio_num; i++) {
		rc = of_property_read_string_index(of_node,
	...
	if (count > 0 && count < MULTIPLEX_GPIO_MAX) {
		ret = of_property_read_u32_array(of_node,
			...
			(unsigned int *)&sensor_info->multiplex_gpio_id,
			count);
	...
	rc = of_property_read_u32(of_node, "huawei,flash_pos_type",
		(u32 *)&sensor_info->flash_pos_type);
	cam_info("%s flash_pos_type 0x%x, rc %d\n",
		__func__, sensor_info->flash_pos_type, rc);
	if (rc < 0) {
		/* 1:mix IR flash ; 0:sigle flash(default) */
		sensor_info->flash_pos_type = 0;
	...
	rc = of_property_read_u32(of_node, "huawei,topology_type",
	...
	struct sensor_power_setting_array *power_setting_array, int is_power_on)
{
	...
	seq_val_name = (is_power_on ?
		"huawei,cam-power-seq-val" :
		"huawei,cam-power-down-seq-val");
	...
	seq_cfg_name = (is_power_on ?
		"huawei,cam-power-seq-cfg-val" :
		"huawei,cam-power-down-seq-cfg-val");
	...
	seq_sensor_index_name = (is_power_on ?
		"huawei,cam-power-seq-sensor-index" :
		"huawei,cam-power-down-seq-sensor-index");
	...
	seq_delay_name = (is_power_on ?
		"huawei,cam-power-seq-delay" :
		"huawei,cam-power-down-seq-delay");
	...
	for (i = 0; i < count; i++) {
		...
		cam_info("%s:%d sensor_index = %d",
			__func__, __LINE__, power_settings[i].sensor_index);
	}
...
{
	cam_debug("%s g_is_fpga=%d", __func__, g_is_fpga);
	return g_is_fpga;
}
