...
_STATIC void eis_pmic_freq_set_curt_sample_thres(int cur_offset)
{
	...
	high = CUR_FREQ_THRES_H * ana2digit / VOL_REF_MV + cur_offset;
	low = CUR_FREQ_THRES_L * ana2digit / VOL_REF_MV + cur_offset;
	if (r_mohm == 1) {
		high_err = 2 * CUR_FREQ_THRES_H_ERR * ana2digit / VOL_REF_MV;
		low_err = 2 * CUR_FREQ_THRES_L_ERR * ana2digit / VOL_REF_MV;
	} else {
		high_err = CUR_FREQ_THRES_H_ERR * ana2digit / VOL_REF_MV;
		low_err = CUR_FREQ_THRES_L_ERR * ana2digit / VOL_REF_MV;
	}
