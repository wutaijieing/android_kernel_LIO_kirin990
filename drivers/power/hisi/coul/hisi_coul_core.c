...
static int uuc_zero_vol = ZERO_V_MAX;
...
static int batt_temp_compensation_en;
static int batt_temp_compensation_r;	/* momh */

/*NTC R-T Table*/
static int t_r_table[][2] = {
...
static void hisi_coul_notify(struct smartstar_coul_device *di, enum hisi_coul_event event);
#ifdef CONFIG_HISI_COUL_POLAR
static void update_polar_params(struct smartstar_coul_device *di, bool update_flag);
...
static int basp_nv_get(char *buffer, const struct kernel_param *kp)
{
	...
	for (i = 0; i < MAX_RECORDS_CNT; i++) {
		(void)snprintf_s(buffer + len, PAGE_SIZE - len, PAGE_SIZE - len - 1, "learned_fcc[%d]:%d\n",
			i, di->nv_info.real_fcc_record[i]);
		len = strlen(buffer);
	}

	(void)snprintf_s(buffer + len, PAGE_SIZE - len, PAGE_SIZE - len - 1, "chargecycles:%d\n",
		di->batt_chargecycles);

	len = strlen(buffer);
	(void)snprintf_s(buffer + len, PAGE_SIZE - len, PAGE_SIZE - len - 1, "latest_record_index:%d\n",
...
static int get_zero_cap_vol(struct smartstar_coul_device *di)
{
	int batt_t = di->batt_temp / 10;
	int zero_vol;

	/* Adjust the zero voltage based on the temperature. */
	if ((batt_t < ZERO_V_ADJ_START_T) && (batt_t > ZERO_V_ADJ_END_T))
		zero_vol = ((ZERO_V_ADJ_START_T - batt_t) * (ZERO_V_MIN - uuc_zero_vol) /
							         (ZERO_V_ADJ_START_T - ZERO_V_ADJ_END_T)) + uuc_zero_vol;
	else if (batt_t <= ZERO_V_ADJ_END_T)
		zero_vol = ZERO_V_MIN;
	else
		zero_vol = uuc_zero_vol;
		      ...
		      int *iavg_ma)
{
	...
	if (di->batt_soc > 30 && di->batt_temp/10 < ZERO_V_ADJ_END_T)
		uuc_pc_step_add = uuc_pc_step_add / 2;
				   ...
				   unsigned int nondc_volt_dec)
{
	...
	coul_core_info("bsoh update policy level %d, nondc_volt_dec %d\n",
...
int adc_to_temp_permille(int temp_volt)
{
	int temprature = 0;
	int i = 0;

	if (temp_volt >= T_V_TABLE[0][1]) {
		return T_V_TABLE[0][0] * PERMILLAGE;
	} else if (temp_volt <= T_V_TABLE[T_V_ARRAY_LENGTH - 1][1]) {
		return T_V_TABLE[T_V_ARRAY_LENGTH - 1][0] * PERMILLAGE;
	} else {
		for (i = 0; i < T_V_ARRAY_LENGTH; i++) {
			if (temp_volt == T_V_TABLE[i][1])
				return T_V_TABLE[i][0] * PERMILLAGE;
			if (temp_volt > T_V_TABLE[i][1]) {
				break;
			}
		}
		...
		if ((T_V_TABLE[i][1] - T_V_TABLE[i - 1][1]) != 0)
			temprature =
				T_V_TABLE[i - 1][0] * PERMILLAGE +
				(long)(temp_volt - T_V_TABLE[i - 1][1]) * PERMILLAGE *
					(T_V_TABLE[i][0] -
					 T_V_TABLE[i - 1][0]) /
...
static long get_ntc_moh_from_temp(int temprature)
{
	...
	for (i = 0; i < ARRAY_SIZE(t_r_table); i++) {
		if (temprature == t_r_table[i][0])
			return (long)t_r_table[i][1] * PERMILLAGE;
	}
...
{
	int len = ARRAY_SIZE(t_r_table);

	temprature = temprature / PERMILLAGE;

	if (temprature < t_r_table[0][0])
		return -1;
	else if (temprature > t_r_table[len - 1][0])
		return -1;

	return 0;
}

static int _coul_battery_temp_compensation(
				int temp_orig_permille, long ibat)
{
	long r_ntc;	/* momh */
	long i_ntc;	/* nA */
	...
	ibat = ibat * MA_2_NA;
	r_compensate = (long long)ibat * batt_temp_compensation_r / i_ntc;
							...
							int temp_orig_permille)
{
	...
	if (batt_temp_compensation_en == 0)
		return temp_orig_permille;
	...
	ibat = (long)-coul_get_battery_current_ma();
	if (ibat < DISCHARGE_CURRENT_OVERHIGH_TH)
		ibat = DISCHARGE_CURRENT_OVERHIGH_TH;
	if (ibat > hisi_bci_get_charge_current_overhigh_th())
		ibat = hisi_bci_get_charge_current_overhigh_th();

	temp_new_permille = _coul_battery_temp_compensation(
						temp_orig_permille, ibat);

	return temp_new_permille;
}

int coul_retry_temp_permille(BATTERY_TEMP_USER_TYPE user)
{
	int retry_times = 3;
	int cnt = 0;
	int t_adc;
	...
	if (di && di->coul_dev_ops && di->coul_dev_ops->get_bat_temp) {
		while (cnt++ < retry_times) {
			...
			if (t_adc < 0) {
				coul_core_err(
					"Bat temp read fail!,retry_cnt = %d\n",
					cnt);
			} else {
...
int coul_battery_temperature_permille(void)
{
...
int coul_ntc_temperature_compensation(struct smartstar_coul_device *di, BATTERY_TEMP_USER_TYPE user)
{
	...
	if ((di->ntc_compensation_is == 1) && !btf_temp_fitted_eqr() &&
...
static void get_ocv_by_vol(struct smartstar_coul_device *di)
{
	...
	hisi_coul_notify(di, HISI_OCV_UPDATE);
}
...
static void get_battery_id_voltage(struct smartstar_coul_device *di)
{
	...
	if ((int)di->nv_info.hkadc_batt_id_voltage == INVALID_BATT_ID_VOL)
		...
		di->batt_id_vol = (unsigned int)di->nv_info.hkadc_batt_id_voltage;
...
static int adjust_soc(struct smartstar_coul_device *di, int soc)
{
	...
	if ((vbat_uv / UA_PER_MA > zero_vol + ZERO_V_COPM_MV) && (di->batt_temp / 10 < ZERO_V_ADJ_START_T))
		goto out;
...
void coul_start_soh_check(void)
{
	...
	static int chging_done_eis_freq_time;
	int acr_time_inc;
	int dcr_time_inc;
	int eis_freq_time_inc;
	static int eis_freq_ntf_cnt = 0;
	...
	if (CHARGING_STATE_CHARGE_DONE == di->charging_state) {
		if (0 == charged_cnt) {
			...
			chging_done_eis_freq_time = hisi_getcurtime() / NSEC_PER_SEC;
		}
		...
		eis_freq_time_inc = charging_done_now_time - chging_done_eis_freq_time;
		...
		if (!di->chg_done_max_avg_cur_flag) {
			...
			if (eis_freq_time_inc > EIS_FREQ_CYCLE_S && eis_freq_ntf_cnt <= SAME_CYC_MAX_TRIAL) {
				chging_done_eis_freq_time = hisi_getcurtime() / NSEC_PER_SEC;
			...
			coul_core_info("acr/dcr/eis_freq notify er, avg cur is more than max !!\n");
		}
	} else {
		charged_cnt = 0;
		eis_freq_ntf_cnt = 0;
...
static void coul_check_drained_battery_flag(struct smartstar_coul_device *di)
{
	...
	ret = snprintf_s(buff, sizeof(buff), sizeof(buff) - 1,
		"last_cycles:%d,cur_cylces:%d,vol=%d,cur=%d\n",
		last_charge_cycles, di->batt_chargecycles, vbat_uv, ibat_ua);
	if (ret < 0) {
...
static void resume_polar_calc(struct smartstar_coul_device *di, int temp)
{
	int delta_temp;
...
int get_fcc_after_cv_adjust(struct smartstar_coul_device *di, int fcc_mah)
{
	int delta_cv_pc;
	...
	if (di->nondc_volt_dec > BASP_FATAL_VDEC_TH) {
		...
		if (!delta_cv_pc)
			delta_cv_pc = PERCENT;
		real_fcc_mah = fcc_mah * PERCENT / delta_cv_pc; /*lint !e414*/
...
void force_ocv_update(struct smartstar_coul_device *di)
{
	if (!di)
		return;
	/* 1500ms for ibat slow down */
	msleep(1500);
				    ...
				    struct iscd_sample_info *sample_info)
{
	...
	if (SUCCESS != iscd_sample_battery_ocv_uv(di, &ocv_uv)) {
		coul_core_err("ISCD sample ocv wrong(%dmV), try to next loop\n", ocv_uv);
		return ERROR;
	}
	if (SUCCESS != iscd_sample_ocv_soc_uAh(di, ocv_uv, &ocv_soc_uAh)) {
...
static int iscd_check_cc_drift(struct smartstar_coul_device *di)
{
	int ret = FALSE;
	static int abn_cc_cnt = 0;
	...
	delta_time = di->iscd->sample_info[di->iscd->size - 1].sample_time.tv_sec -
		di->iscd->sample_info[di->iscd->size - 2].sample_time.tv_sec;
	/* recharging happening in 30 min after charging done is abnormal */
	if (di->iscd->sample_info[di->iscd->size - 2].sample_cnt == 1 &&
		di->iscd->sample_info[di->iscd->size - 1].sample_cnt == 1 &&
		delta_time < 2 * ISCD_CALC_INTERVAL_900S) {
		abn_cc_cnt++;
		ret = TRUE;
	} else {
		abn_cc_cnt = 0;
	...
	if (!dmd_reported && abn_cc_cnt >= ISCD_DMD_REPORT_TIME_FOR_ABN_RECHARGE) {
		coul_core_err("Abnormal charging detected for more than 3 times, do dmd report\n");
		if (snprintf_s(dsm_buff, ISCD_DSM_LOG_SIZE_MAX,
			ISCD_DSM_LOG_SIZE_MAX - 1, "Abnormal cc drift detected for %d times\n", abn_cc_cnt) < 0)
			coul_core_err("dmd content buffer overflow, report abandoned\n");
		else
			power_dsm_dmd_report(POWER_DSM_BATTERY,
				di->iscd->level_config[di->iscd->total_level - 1].dsm_err_no, dsm_buff);
		memset_s(dsm_buff, sizeof(dsm_buff), 0x00, sizeof(dsm_buff));
		dmd_reported = TRUE;
	}
...
static int iscd_remove_invalid_samples(struct smartstar_coul_device *di)
{
	...
	if (iscd_check_cc_drift(di)) {
		coul_core_err("Abnormal recharge detected.\n");
		iscd_clear_sampled_info(di);
		iscd_reset_isc_buffer(di);
		di->iscd->rm_bcd = 0;
		di->iscd->fcc_bcd = 0;
...
void iscd_dump_dsm_info(struct smartstar_coul_device *di, char *buf)
{
	...
	di->qmax = coul_get_qmax(di);
	if (snprintf_s(tmp_buf, (size_t)ISCD_ERR_NO_STR_SIZE, (size_t)(ISCD_ERR_NO_STR_SIZE - 1),
		"nondc_volt_dec is %d mv\n", di->nondc_volt_dec) < 0)
		return;
	if (strncat_s(buf, ISCD_ERR_NO_STR_SIZE, tmp_buf, strlen(tmp_buf)) != EOK)
		return;
...
static void iscd_work(struct work_struct *work)
{
	...
	if (CHARGING_STATE_CHARGE_STOP == di->charging_state) {
		...
		if (SUCCESS == iscd_sample_battery_info(di, sample_info)) {
			iscd_insert_sampled_info(di, sample_info);
			ret = iscd_remove_invalid_samples(di);
			if (TRUE == iscd_calc_short_current(di, ret)) {
				if (ISCD_VALID == iscd_is_short_current_valid(di))
					iscd_process_short_current(di);
			}
			if (ENABLED == di->iscd->enable)
...
static void coul_core_get_iscd_info(struct device_node* np, struct iscd_info *iscd)
{
	...
	ret = of_property_read_s32(np, "iscd_file_magic_num", &iscd->iscd_file_magic_num);
	if (ret) {
		coul_core_err("get iscd_file_magic_num fail, use default one !!\n");
		iscd->iscd_file_magic_num = FATAL_ISC_MAGIC_NUM;
	}
	coul_core_info("ISCD iscd_file_magic_num = %d\n", iscd->iscd_file_magic_num);
...
static void get_uuc_vol_dts(struct device_node *np)
{
	...
	if (of_property_read_u32(np, "uuc_zero_vol", (u32 *)&uuc_zero_vol))
		uuc_zero_vol = ZERO_V_MAX;

	if (uuc_zero_vol < ZERO_V_MIN)
		uuc_zero_vol = ZERO_V_MIN;

	coul_core_info("uuc_zero_vol:%d\n", uuc_zero_vol);
}

static void get_batt_temp_compensation_dts(struct device_node *np)
{
	if (of_property_read_u32(np, "batt_temp_compensation_en",
		    &batt_temp_compensation_en)) {
		coul_core_err("error:get batt_temp_compensation_en failed!\n");
		batt_temp_compensation_en = 0;
	}

	if (of_property_read_u32(np, "batt_temp_compensation_r",
		    &batt_temp_compensation_r)) {
		coul_core_err("error:get batt_temp_compensation_r failed!\n");
		batt_temp_compensation_r = 0;
	}
	coul_core_info("batt_temp_compensation_en:%d, compensation_r %d\n",
		batt_temp_compensation_en, batt_temp_compensation_r);
}
...
static int  hisi_coul_probe(struct platform_device *pdev)
{
	...
	coul_core_info("%s: batt ID is %d\n", __FUNCTION__, di->batt_id_vol);
	coul_core_debug("%s: batt_brand is %s\n", __FUNCTION__, di->batt_data->batt_brand);
...
coul_no_battery:
	...
	coul_ops->get_battery_temp_permille = coul_battery_temperature_permille;
	di->ops = coul_ops;
...
void hisi_coul_notify(struct smartstar_coul_device *di, enum hisi_coul_event event)
{
	struct ocv_update_data data;
	int ret;

	switch(event) {
	case HISI_OCV_UPDATE:
		data.sample_time_sec = current_kernel_time().tv_sec;
		data.ocv_volt_uv = di->batt_ocv;
		coul_cal_uah_by_ocv(di->batt_ocv, &data.ocv_soc_uah);
		data.cc_value = di->iscd_full_update_cc;
		data.tbatt = di->batt_temp;
		data.pc = interpolate_pc_high_precision(di->batt_data->pc_temp_ocv_lut,
			di->batt_temp, di->batt_ocv / UVOLT_PER_MVOLT);
		data.ocv_level = di->last_ocv_level;
		data.batt_chargecycles = di->batt_chargecycles;
		ret = hisi_call_coul_blocking_notifiers(HISI_OCV_UPDATE, (void *)&data);
...
static int hisi_coul_resume(struct platform_device *pdev)
{
	...
	resume_polar_calc(di, di->batt_temp);
	start_polar_sample();
#endif
