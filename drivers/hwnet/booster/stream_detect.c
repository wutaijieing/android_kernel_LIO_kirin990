...
static bool is_video_stream(u8 *str, u32 len, struct stream_info info)
{
	u32 cnt;
...
static bool is_target_stream(u8 *str, u32 len, struct stream_info info)
{
	if (g_stream_ctx->stream_type == VEDIO)
		return is_video_stream(str, len, info);
	else
		return false;
}
...
void stream_process_hook_in(struct sk_buff *skb, u16 family)
{
	...
	pos_selec = process_new_stream(stream, info, tcp_data, tcp_len);
	if (pos_selec == NULL)
		goto unlock;
	...
	struct sk_buff *skb, struct sock *sk, u32 tcp_len)
{
	...
	if (skt == NULL)
		return;
	prior_snd_una = skt->snd_una;
	ack = ntohl(th->ack_seq);
	prior_packets = skt->packets_out; // Packets which are "in flight"
	if ((!prior_packets) || (tcp_len > skt->tcp_header_len)) { // no ack && no data
		is_dupack = false;
...
void stream_process_hook_out(struct sk_buff *skb, u16 family)
{
	...
	pos_selec = find_match_entry(stream, info);
	if (pos_selec != NULL)
		update_tcp_out_para(&pos_selec->stat, skb, sk, tcp_len);
	spin_unlock_bh(&stream->lock);
}
