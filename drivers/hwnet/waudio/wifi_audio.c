	...
	void *ptr)
{
	...
	case NETDEV_CHANGENAME:
		wifi_audio_log_info("netdev notifier event handle");
...
static void netlink_event_receive(struct sk_buff *netlink_skb)
{
	...
	if(msg_rev.id == NETLINK_MSG_SET_USER_ID) {
		...
		wifi_audio_log_info("set data msg_rev.id %d", msg_rev.id);
...
int netlink_event_report(const struct netlink_data *event)
{
	...
	wifi_audio_log_info("id = %d, len = %u", event->id, event->msg_len);
