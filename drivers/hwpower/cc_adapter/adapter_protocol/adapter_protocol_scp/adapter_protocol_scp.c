...
static int scp_protocol_check_adp_type(int mode, int value)
{
	...
	if ((mode & ADAPTER_SUPPORT_SCP_B_LVC) &&
		...
		/* check 11v6a */
		case SCP_PROTOCOL_ADP_B_TYPE1_66W:
...
static int scp_protocol_get_support_mode(int *mode)
{
	...
	if (l_dev->info.detect_finish_flag)
		*mode = l_dev->info.support_mode;
		...
		scp_protocol_detect_adapter_support_mode(mode);
...
static int scp_protocol_set_output_mode(int enable)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_OUTPUT_MODE_SHIFT) &
...
static int scp_protocol_set_reset(int enable)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_RESET_SHIFT) &
...
static int scp_protocol_set_output_enable(int enable)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_OUTPUT_EN_SHIFT) &
...
static int scp_protocol_set_dp_delitch(int time)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_DP_DELITCH_SHIFT) &
...
static int scp_protocol_set_watchdog_timer(int second)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_WATCHDOG_SHIFT) &
...
static int scp_protocol_set_usbpd_enable(int enable)
{
	...
	value[0] |= ((int)(tmp_value << SCP_PROTOCOL_USBPD_ENABLE_SHIFT) &
...
static struct adapter_protocol_ops adapter_protocol_scp_ops = {
	...
	.set_output_mode = scp_protocol_set_output_mode,
