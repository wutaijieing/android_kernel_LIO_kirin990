...
static bool hisi_npu_is_low_temperature(void)
{
#ifdef CONFIG_HISI_THERMAL_TSENSOR
	int temp = 0;
	int ret;
...
int hisi_npu_voltage_step_up(int cur_pid, int tar_pid, int temp_stat)
{
	...
	for (module = 0; module < g_dvfs_data.module_count; module++) {
		...
		} else if (temp_stat == TEMP_NORMAL ||
			   temp_stat == TEMP_LOW_EXIT) {
			profile = module_profile->profile_table;
#ifdef CONFIG_HISI_NPU_AVS_VDM
			if ((profile[tar_pid].func_sw & PROF_AVS) > 0) {
				if (((unsigned int)cur_pid) >=
				    module_profile->pa_ctrl.pa_safe_prof)
					pa_sw = 1;

				pa_volt = npu_cluster_avs(g_dvfs_data.main_module,
							  pa_sw);
		...
		if (tar_voltage > last_voltage) {
			...
			buck_delay_time =
				(tar_voltage - last_voltage) / buck_step_time +
	...
	for (module = 0; module < g_dvfs_data.module_count; module++) {
		...
		if (temp_stat == TEMP_LOW_ENTER) {
			...
			if ((profile[tar_pid].func_sw & PROF_AVS) > 0) {
				...
				tar_voltage = npu_volt_cfg2vol(profile[tar_pid].avsvolt[temp_idx]) +
					      pa_volt;
				pr_debug("[npu pasnsor] pa%u cur_pid%d sw%u\n",
					 pa_volt, cur_pid, pa_sw);
			} else {
				tar_voltage = profile[tar_pid].voltage;
			}
		...
		if (tar_voltage < last_voltage) {
			...
			buck_delay_time =
				(last_voltage - tar_voltage) / buck_step_time +
				module_profile->buck_hold_time;
...
int hisi_npu_voltage_update(int cur_pid, int tar_pid, int temp_stat)
{
	...
	for (module = 0; module < g_dvfs_data.module_count; module++) {
		...
		if (last_voltage > tar_voltage)
			udelay((last_voltage - tar_voltage) / buck_step_time +
			       module_profile->buck_hold_time);
		else
			udelay((tar_voltage - last_voltage) / buck_step_time +
			       module_profile->buck_hold_time);
	}
