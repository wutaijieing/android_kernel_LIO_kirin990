					...
					struct hisi_npu_module_profile *module_profile)
{
	...
	void __iomem *base[MAX_NPU_PM_REG] = { NULL, NULL, NULL };
			       ...
			       const char *main_module_name)
{
	...
	struct module_npu_pa_ctrl_info *pa_ctrl = &module_profile->pa_ctrl;
	struct module_svfd_ctrl_info *svfd_ctrl = &module_profile->svfd_ctrl;
	int profile_count = module_profile->profile_count;
	int max_avs = profile_count * MAX_AVS_VOLT;
	...
	prof_switch = devm_kzalloc(dev, sizeof(unsigned int) * profile_count,
				   GFP_KERNEL);
	if (prof_switch == NULL) {
	...
	svfd_code = devm_kzalloc(dev, sizeof(unsigned int) * max_avs,
				 GFP_KERNEL);
	if (svfd_code == NULL) {
	...
	avs_volt = devm_kzalloc(dev, sizeof(unsigned int) * max_avs,
				GFP_KERNEL);
	if (avs_volt == NULL) {
	...
	ret += of_property_read_u32_array(module_np, "svfd-reg-offset",
					  (u32 *)(svfd_ctrl->vdm_reg_offset),
					  SVFD_REG_OFFSET_NUM);

	ret += of_property_read_u32_array(module_np, "svfd-cfg-bit",
					  (u32 *)(svfd_ctrl->vdm_cfg_bit),
					  SVFD_CFG_BIT_NUM);
	...
	ret += of_property_read_u32_array(module_np, "prof-svfdcode",
					  (u32 *)(svfd_code), max_avs);

	ret += of_property_read_u32_array(module_np, "prof-avsvolt",
					  (u32 *)(avs_volt), max_avs);
