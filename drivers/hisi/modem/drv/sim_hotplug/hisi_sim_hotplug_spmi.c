...
void sim_jtag_shared_mem_write(void)
{
    ...
    if(SHM_BASE_ADDR != 0) {
        ...
        SIMHP_LOGE("SHM base addr: 0x%x, simjtag offset: 0x%x.\n", SHM_BASE_ADDR, SHM_OFFSET_SIMJTAG);
...
int set_gpio_det_fun(struct hisi_sim_hotplug_info *info)
{
    ...
    if (info->sd_io_cfg.virt_addr != NULL) {
        ...
        SIMHP_LOGE("set %x to %x.\n", value & 0xfffffffc | 0x01,  (uintptr_t)info->sd_io_cfg.virt_addr + info->sd_io_cfg.esim_det_ctrl_offset[0]);
        ...
        SIMHP_LOGE("set %x to %x.\n", value & 0xfffffffc | 0x02,  (uintptr_t)info->sd_io_cfg.virt_addr + info->sd_io_cfg.esim_det_ctrl_offset[1]);
        ...
        SIMHP_LOGE("set %x to %x.\n", value & 0xfffffffc | 0x02,  (uintptr_t)info->sd_io_cfg.virt_addr + info->sd_io_cfg.esim_det_ctrl_offset[2]);
        ...
        SIMHP_LOGE("set %x to %x.\n", value & 0xfffffffc | 0x01,  (uintptr_t)info->sd_io_cfg.virt_addr + info->sd_io_cfg.esim_det_ctrl_offset[3]);
