		     ...
		     union timestamp_info *timestamp_value)
{
	...
	value_short = (timestamp_str[0] - '0') * THOUSANDS_DIGIT_WEIGHT +
		(timestamp_str[1] - '0') * HUNDREDS_DIGIT_WEIGHT +
		(timestamp_str[2] - '0') * TENS_DIGIT_WEIGHT +
		(timestamp_str[3] - '0');
	timestamp_value->timestamp.year = value_short;

	/* month: 1~12, string to int */
	value_char = (timestamp_str[5] - '0') * TENS_DIGIT_WEIGHT +
			(timestamp_str[6] - '0');
	timestamp_value->timestamp.month = value_char;

	/* day: 1~31, string to int */
	value_char = (timestamp_str[8] - '0') * TENS_DIGIT_WEIGHT +
			(timestamp_str[9] - '0');
	timestamp_value->timestamp.day = value_char;

	/* hour: 0~23, string to int */
	value_char = (timestamp_str[11] - '0') * TENS_DIGIT_WEIGHT +
			(timestamp_str[12] - '0');
	timestamp_value->timestamp.hour = value_char;

	/* minute: 0~59, string to int */
	value_char = (timestamp_str[14] - '0') * TENS_DIGIT_WEIGHT +
			(timestamp_str[15] - '0');
	timestamp_value->timestamp.minute = value_char;

	/* second: 0~59, string to int */
	value_char = (timestamp_str[17] - '0') * TENS_DIGIT_WEIGHT +
			(timestamp_str[18] - '0');
...
static int _get_image_count(struct img_file_info *file,
...
static int _check_sub_file_info(struct img_file_info *file, unsigned int total_size)
{
...
static int hisee_image_type_chk(enum hisee_img_file_type type, char *sub_file_name,
				struct hisee_img_header *p_hisee_img_head,
				unsigned int *index)
{
	unsigned int i;
	int ret = HISEE_OK;
