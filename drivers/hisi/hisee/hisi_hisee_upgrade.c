...
{
	/* 2016: the beginning year of the feature life */
	if (timestamp_value->timestamp.year < 2016)
		return HISEE_FALSE;

	/* 2050: the deadline of the feature life */
	if (timestamp_value->timestamp.year >= 2050)
		return HISEE_FALSE;

	if (timestamp_value->timestamp.month > 12) /* max months in a year */
		return HISEE_FALSE;
	 ...
	 */
	if (timestamp_value->timestamp.day > 31) /* max days in a month */
		return HISEE_FALSE;

	if (timestamp_value->timestamp.hour >= 24) /* max hours in a day */
		return HISEE_FALSE;

	if (timestamp_value->timestamp.minute >= 60) /* max minutes in a hour */
		return HISEE_FALSE;

	if (timestamp_value->timestamp.second >= 60) /* max seconds in a minute */
		return HISEE_FALSE;
