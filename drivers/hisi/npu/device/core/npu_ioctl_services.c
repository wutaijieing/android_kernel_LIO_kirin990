...
{
	int ret;
	npu_workmode_info_t workmode_info = {0};
	...
	ret = copy_from_user_safe(&workmode_info, (void __user *)(uintptr_t)arg, sizeof(npu_workmode_info_t));
	if (ret != 0) {
		NPU_DRV_ERR("fail to copy sec_mode_info params, ret = %d\n", ret);
		return -EINVAL;
	}
	COND_RETURN_ERROR(workmode_info.work_mode >= NPU_WORKMODE_MAX, -EINVAL,
		"invalid work_mode = %u\n", workmode_info.work_mode);

	/* hwts aicore pool switch */
	dev_ctx->pm.switch_info.work_mode = workmode_info.work_mode;
	dev_ctx->pm.switch_info.action = POWER_ON; /* 1:poweron, 0:poweroff */
	dev_ctx->pm.switch_info.flags = workmode_info.flags;

	NPU_DRV_DEBUG("workmode = %u\n", workmode_info.work_mode);
	ret = npu_pm_enter_workmode(proc_ctx, dev_ctx, workmode_info.work_mode);
	if (ret != 0) {
		NPU_DRV_ERR("fail to enter workwode: %u, ret = %d\n", workmode_info.work_mode, ret);
		return -EINVAL;
	}
...
int devdrv_ioctl_exit_workwode(struct devdrv_proc_ctx *proc_ctx, unsigned long arg)
{
	...
	dev_ctx->pm.switch_info.work_mode = workmode;
	dev_ctx->pm.switch_info.action = POWER_OFF; /* 1:poweron, 0:poweroff */
	dev_ctx->pm.switch_info.flags = 0; /* default value no useful purpose */
...
int devdrv_ioctl_switch_hwts_pool(struct devdrv_proc_ctx *proc_ctx, struct devdrv_dev_ctx *dev_ctx, u64 arg)
{
	int ret;
	hwts_switch_pool_info_t switch_info = {0};

	ret = copy_from_user_safe(&switch_info, (void __user *)(uintptr_t)arg, sizeof(hwts_switch_pool_info_t));
	if (ret != 0) {
		NPU_DRV_ERR("fail to copy switch params, ret = %d\n", ret);
		return -EINVAL;
	}

	mutex_lock(&dev_ctx->npu_power_up_off_mutex);
	NPU_DRV_WARN("secure_mode = 0x%x, action = 0x%x,  flags = 0x%x\n",
		switch_info.work_mode, switch_info.action, switch_info.flags);
	ret = npu_plat_switch_hwts_aicore_pool(dev_ctx, &switch_info);
	if (ret != 0)
		NPU_DRV_ERR("switch hwts aicore pool fail, ret = 0x%x\n", ret);
	...
	struct devdrv_dev_ctx *dev_ctx)
{
	...
	case DEVDRV_IOC_LOAD_MODEL_BUFF: // for sink stream(v200)
		...
		break;
	case DEVDRV_IOC_SWITCH_HWTS_POOL:
		ret = devdrv_ioctl_switch_hwts_pool(proc_ctx, dev_ctx, custom_para->arg);
		break;
	default:
