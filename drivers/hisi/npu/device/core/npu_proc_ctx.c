	...
	u16 strategy, u16 priority)
{
	...
	*stream_info = devdrv_alloc_stream(strategy, stream_sq_id, stream_cq_id);
	...
	stream_sub_info = (struct npu_id_entity *)((*stream_info)->stream_sub);
	if (strategy & STREAM_STRATEGY_SINK) {
...
bool devdrv_proc_is_valid_report_received(struct devdrv_ts_cq_info *cq_info)
{
	int report_phase;

	report_phase = __devdrv_get_report_phase_from_cq_info(cq_info, RREPORT_FROM_CQ_TAIL);

	NPU_DRV_DEBUG("cqid = %d cqtail = %d cq_info phase = %d report phase = %d\n",
...
int devdrv_proc_report_wait(struct devdrv_proc_ctx *proc_ctx, struct devdrv_ts_cq_info *cq_info, int timeout)
{
	...
	if (wait_time == -1) {
		while (1) {
			...
			if (devdrv_proc_is_valid_report_received(cq_info) == false) {
				proc_ctx->cq_tail_updated = CQ_HEAD_INITIAL_UPDATE_FLAG;
				continue;
			}
	...
	while (wait_time >= wait_event_once) {
		...
		if (devdrv_proc_is_valid_report_received(cq_info) == true) {
			return 1;
		} else {
...
int devdrv_proc_report_sync_wait(struct devdrv_proc_ctx *proc_ctx, struct devdrv_report_sync_wait_info *wait_info)
{
	...
	stream_sub = (struct npu_id_entity *)(stream_info->stream_sub);
