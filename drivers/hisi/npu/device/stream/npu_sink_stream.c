...
int devdrv_sink_stream_list_init(struct devdrv_dev_ctx *dev_ctx)
{
	...
	for (i = 0; i < sink_stream_num; i++) {
		...
		id_allocator = i < DEVDRV_MAX_SINK_LONG_STREAM_ID ? id_allocator_long : id_allocator_short;
		idx = i < DEVDRV_MAX_SINK_LONG_STREAM_ID ? i : (i - DEVDRV_MAX_SINK_LONG_STREAM_ID);
		if (id_allocator != NULL && id_allocator->id_entity_base_addr != NULL)
			sink_stream_info->stream_sub =
			(void *)((uintptr_t)id_allocator->id_entity_base_addr + id_allocator->entity_size * idx);
		else
			continue;
