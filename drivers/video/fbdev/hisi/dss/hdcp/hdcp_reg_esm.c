...
static int hdcp_esm_intr_en(struct dp_ctrl *dptx)
{
	...
	if (!dptx || !dptx->hisifd) {
		HISI_FB_ERR("%s dptx or hisifd is null\n", HDCP_TAG);
	...
	if (dptx->hisifd->secure_ctrl.hdcp_dpc_sec_en)
		...
		HISI_FB_ERR("%s ATF:hdcp_dpc_sec_en is NULL\n", HDCP_TAG);

	/* enable interrupt include HDCP22 GPIO output status */
	if(dptx->hisifd->secure_ctrl.hdcp_reg_get)
		reg = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_API_INT_MSK);
	else
		HISI_FB_ERR("%s ATF:hdcp_reg_get is NULL\n", HDCP_TAG);

	if(dptx->hisifd->secure_ctrl.hdcp_int_mask)
		dptx->hisifd->secure_ctrl.hdcp_int_mask(reg);
	else
		HISI_FB_ERR("%s ATF:hdcp_int_mask is NULL\n", HDCP_TAG);
...
static int hdcp_esm_handle_cp_irq(struct dp_ctrl *dptx)
{
	...
	if (dptx->hisifd->secure_ctrl.hdcp_cp_irq)
		...
		HISI_FB_ERR("%s ATF:hdcp_cp_irq is NULL\n", HDCP_TAG);
...
static void hdcp22_gpio_intr_clear(bool keep_intr, struct dp_ctrl *dptx, uint32_t hdcpobs)
{
	...
	if (keep_intr) { /* mask interrupt: DPTX_HDCP22_GPIOINT */
		HISI_FB_DEBUG("%s HDCP2.2 register mask\n", HDCP22_TAG);
		if(dptx->hisifd->secure_ctrl.hdcp_reg_get)
			reg_mask = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_API_INT_MSK);
		else
			HISI_FB_ERR("%s ATF:hdcp_reg_get is NULL\n", HDCP22_TAG);
	...
	HISI_FB_DEBUG("%s HDCP2.2 register clear, DPTX_HDCP_OBS=0x%x\n", HDCP22_TAG, hdcpobs);
	if (dptx->hisifd->secure_ctrl.hdcp_obs_set)
		dptx->hisifd->secure_ctrl.hdcp_obs_set(hdcpobs);
	else
		HISI_FB_ERR("%s ATF:hdcp_obs_set is NULL\n", HDCP22_TAG);
}
...
static void handle_hdcp22_gpio_intr(struct dp_ctrl *dptx)
{
	...
	if (dptx->hisifd->secure_ctrl.hdcp_reg_get)
		hdcpobs = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_OBS);
	else
		HISI_FB_ERR("%s ATF:hdcp_reg_get is NULL\n", HDCP22_TAG);

	HISI_FB_DEBUG("%s DPTX_HDCP_OBS = 0x%x\n", HDCP22_TAG, hdcpobs);
	if (hdcpobs & HDCP22_BOOTED)
		HISI_FB_DEBUG("%s ESM has booted\n", HDCP22_TAG);

	if (hdcpobs & HDCP22_CAP_CHECK_COMPLETE) {
		if (hdcpobs & HDCP22_CAPABLE_SINK)
			HISI_FB_DEBUG("%s sink is HDCP22 capable\n", HDCP22_TAG);
		else
			HISI_FB_NOTICE("%s sink is not HDCP22 capable\n", HDCP22_TAG);
	}

	if (hdcpobs & HDCP22_AUTH_SUCCESS) {
		keep_intr = true;
		HISI_FB_NOTICE("%s the authentication is succeded\n", HDCP22_TAG);
		dp_imonitor_set_param(DP_PARAM_HDCP_KEY_S, NULL);
	}

	if (hdcpobs & HDCP22_AUTH_FAILED) {
		keep_intr = true;
		HISI_FB_NOTICE("%s the authentication is failed\n", HDCP22_TAG);
		dp_imonitor_set_param(DP_PARAM_HDCP_KEY_F, NULL);
	}

	if (hdcpobs & HDCP22_RE_AUTH_REQ)
		HISI_FB_WARNING("%s the sink has requested a re-authentication\n", HDCP22_TAG);
...
static void hdcp_intr_clear(bool keep_intr, struct dp_ctrl *dptx, uint32_t hdcpintsts)
{
	...
	if (keep_intr) { /* mask interrupt: (DPTX_HDCP_ENGAGED | DPTX_HDCP_FAILED) */
		HISI_FB_DEBUG("%s HDCP1.3 register mask\n", HDCP13_TAG);
		if (dptx->hisifd->secure_ctrl.hdcp_reg_get)
			reg_mask = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_API_INT_MSK);
		else
			HISI_FB_ERR("%s ATF:hdcp_reg_get is NULL\n", HDCP13_TAG);
	...
	HISI_FB_DEBUG("%s HDCP1.3 and 2.2 register clear. DPTX_HDCP_INT_STS=0x%x\n", HDCP13_TAG, hdcpintsts);
	if (dptx->hisifd->secure_ctrl.hdcp_int_clr)
		dptx->hisifd->secure_ctrl.hdcp_int_clr(hdcpintsts);
	else
		HISI_FB_ERR("%s ATF:hdcp_int_clr is NULL\n", HDCP13_TAG);
}
...
static int hdcp_esm_handle_intr(struct dp_ctrl *dptx, uint32_t *auth_fail_count)
{
	...
	if (!dptx || !auth_fail_count || !dptx->hisifd) {
		HISI_FB_ERR("%s dptx or hisifd or fail_count is NULL\n", HDCP13_TAG);
		return RET_HDCP_ERROR;
	}

	if(dptx->hisifd->secure_ctrl.hdcp_reg_get)
		hdcpintsts = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_INT_STS);
	else
		HISI_FB_ERR("%s ATF:hdcp_reg_get is NULL\n", HDCP13_TAG);

	HISI_FB_DEBUG("%s DPTX_HDCP_INT_STS=0x%08x\n", HDCP13_TAG, hdcpintsts);
	if (hdcpintsts & HDCP_KSV_ACCESS_INT)
		HISI_FB_DEBUG("%s KSV memory access guaranteed for read, write access\n", HDCP13_TAG);

	if (hdcpintsts & HDCP_KSV_SHA1)
		HISI_FB_DEBUG("%s SHA1 verification has been done\n", HDCP13_TAG);

	if (hdcpintsts & HDCP_AUX_RESP_TIMEOUT) {
		ret = RET_HDCP13_AUTH_TIMEOUT;
		keep_intr = true;
		HISI_FB_WARNING("%s DPTX_HDCP_AUX_RESP_TIMEOUT\n", HDCP13_TAG);
	}
	...
	if (hdcpintsts & HDCP_FAILED) {
		...
		if (*auth_fail_count > DPTX_HDCP_MAX_AUTH_RETRY) {
			...
			keep_intr = true;
			HISI_FB_ERR("%s Reach max allowed retries count=%u\n", HDCP13_TAG, *auth_fail_count);
		} else {
			HISI_FB_INFO("%s HDCP13 authentication process was failed!\n", HDCP13_TAG);
			dp_imonitor_set_param(DP_PARAM_HDCP_KEY_F, NULL);
		}
	}
	...
	if (hdcpintsts & HDCP_ENGAGED) {
		*auth_fail_count = 0;
		...
		keep_intr = true;
		HISI_FB_NOTICE("%s HDCP13 authentication process was successful\n", HDCP13_TAG);
		dp_imonitor_set_param(DP_PARAM_HDCP_KEY_S, NULL);
	}

	if (hdcpintsts & HDCP22_GPIOINT) {
		HISI_FB_DEBUG("%s HDCP22_GPIOINT\n", HDCP22_TAG);
...
static int hdcp_check_status(struct dp_ctrl *dptx)
{
	...
	if (dptx->hisifd->secure_ctrl.hdcp_reg_get == NULL) {
		HISI_FB_ERR("%s ATF:hdcp_reg_get is null\n", HDCP_TAG);
		return RET_HDCP_ERROR;
	}
	temp_value = dptx->hisifd->secure_ctrl.hdcp_reg_get(HDCP_CFG);
	HISI_FB_DEBUG("%s DPTX_HDCP_CFG is:0x%x!\n", HDCP_TAG, temp_value);
