...
static void dss_pdp_isr_vactive0_start_handle(struct hisi_fb_data_type *hisifd, uint32_t isr_s2)
{
	...
	if (hisifd->effect_info[hisifd->panel_info.disp_panel_id].dpp_cmdlist_type == DPP_CMDLIST_TYPE_COMMON)
		hisi_dpp_cmdlist_action(hisifd);
#endif
...
#ifdef CONFIG_HISI_DPP_CMDLIST
			if (hisifd->effect_info[disp_panel_id].dpp_cmdlist_type == DPP_CMDLIST_TYPE_COMMON) {
				hisi_dpp_cmdlist_action(hisifd);
			}
			else if(hisifd->effect_info[disp_panel_id].dpp_cmdlist_type == DPP_CMDLIST_TYPE_ROI) {
...
irqreturn_t dss_dsi0_isr(int irq, void *ptr)
{
	...
	if (isr_s2 & BIT_VACTIVE_CNT) {
		if (hisifd->effect_info[hisifd->panel_info.disp_panel_id].dpp_cmdlist_type == DPP_CMDLIST_TYPE_ROI)
