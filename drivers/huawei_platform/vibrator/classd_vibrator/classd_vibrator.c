...
static bool is_support_vsys(struct device_node *dn)
{
	const char *vsys_support_str = NULL;
	int ret;
	...
	ret = of_property_read_string(dn, "vsys_support", &vsys_support_str);
	if (ret != 0) {
		hwlog_info("%s read vsys support fail, may not use, ret = %d", __func__, ret);
		return false;
	}

	if (strncmp(DTS_ON, vsys_support_str, sizeof(DTS_ON)) == 0) {
		hwlog_info("%s, vibrator vsys support", __func__);
		return true;
	} else if (strncmp(DTS_OFF, vsys_support_str, sizeof(DTS_OFF)) == 0) {
		hwlog_info("%s, vibrator vsys not support", __func__);
		return false;
	} else {
		hwlog_warn("%s, invalid vsys config", __func__);
		return false;
	}
}
...
static int classd_vibrator_parse_dts_param(struct classd_vibrator_data *priv)
{
	...
	priv->vsys_support = is_support_vsys(dn);
			...
			size_t len, loff_t *off)
{
	...
	if (type >= MIN_CALIBRATOIN_NUM && type <= MAX_CALIBRATOIN_NUM)
		type = (type - MIN_CALIBRATOIN_NUM) + PRE_EFFECT_NUM;
...
static void vibrator_remove_workqueue(void)
{
	if (g_haptics_data->vibrator_queue) {
		cancel_delayed_work(&g_haptics_data->work_play_eff);
...
static int classd_vibrator_probe(struct platform_device *pdev)
{
	...
	if (priv->vib_type == VIB_DSP_PCM_TYPE) {
		...
		change_vibrator_mode(priv->vsys_support);
		init_trans_buffer();
		if (g_trans_buffer == NULL) {
