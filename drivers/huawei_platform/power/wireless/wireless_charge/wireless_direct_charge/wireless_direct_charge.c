...
static int wldc_get_bat_voltage(void)
{
	...
	btb_vol = dc_get_bat_btb_voltage_with_comp(di->ic_data.cur_type,
...
static int wldc_get_bat_current(void)
{
	...
	ret = dc_get_bat_current_with_comp(di->ic_data.cur_type, &bat_curr);
	if (ret) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		wldc_fill_eh_buf(di->wldc_err_dsm_buff,
...
int wldc_get_ls_vbus(void)
{
	...
	ret = dc_get_vbus_voltage(di->ic_data.cur_type, &vbus);
	if (ret < 0) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		wldc_fill_eh_buf(di->wldc_err_dsm_buff,
...
static int wldc_get_ls_ibus(void)
{
	...
	ret = dc_get_ic_ibus(di->ic_data.cur_type, &ibus);
	if (ret < 0) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		wldc_fill_eh_buf(di->wldc_err_dsm_buff,
...
int wldc_chip_init(struct wldc_dev_info *di)
{
	...
	ret = dc_batinfo_init(di->ic_data.cur_type);
	if (ret) {
...
static void wldc_charge_regulation(struct wldc_dev_info *di)
{
	...
	hwlog_info("cur_stage=%d, ls: vbus=%d ibus=%d vbatt=%d ibatt=%d,\t"
		...
		di->cur_stage, data.ls_vbus, data.ls_ibus, data.ls_vbatt,
		data.ls_ibatt, data.rx_ratio, data.rx_vrect, data.rx_vout,
		data.rx_iout, data.rx_iout_avg, data.cp_iout, data.cp_iout_avg,
		di->cur_vbat_hth, di->cur_cp_iout_hth,
...
void wldc_stop_charging(struct wldc_dev_info *di)
{
	...
	ret = dc_batinfo_exit(di->ic_data.cur_type);
	if (ret)
...
static int wldc_multi_ic_check(struct wldc_dev_info *di)
{
	...
	rx_ratio = di->mode_para[di->cur_dc_mode].init_para.rx_ratio;
	if (mulit_ic_check(di->ic_data.cur_type, &di->ic_check_info, rx_ratio) < 0) {
