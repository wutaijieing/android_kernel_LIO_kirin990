...
static struct direct_charge_ic_ops rt9748_sysinfo_ops = {
	...
	.config_ic_watchdog = rt9748_watchdog_config,
...
static void rt9748_irq_work(struct work_struct *work)
{
	...
	if (rt9748_interrupt_notify_enable_flag == RT9748_ENABLE_INTERRUPT_NOTIFY) {
	...
	const struct i2c_device_id *id)
{
	...
	ret = dc_lvc_ops_register(di->ic_role, &rt9748_sysinfo_ops);
	if (ret) {
		hwlog_err("rt9748 sysinfo ops register fail\n");
		goto rt9748_fail_3;
	}

	ret = dc_batinfo_ops_register(di->ic_role, &rt9748_batinfo_ops);
	if (ret) {
