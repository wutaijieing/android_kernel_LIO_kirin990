...
static int direct_charge_get_bat_voltage(int *value)
{
	...
	btb_vol = dc_get_bat_btb_voltage_with_comp(l_di->cur_mode,
...
int direct_charge_get_bat_current(int *value)
{
	...
	if (dc_get_bat_current_with_comp(l_di->cur_mode, value)) {
		direct_charge_fill_eh_buf(l_di->dsm_buff,
			sizeof(l_di->dsm_buff),
			DC_EH_GET_IBAT, NULL);
		direct_charge_set_stop_charging_flag(1);
...
int direct_charge_get_device_vbus(int *value)
{
	...
	if (dc_get_vbus_voltage(l_di->cur_mode, value)) {
		direct_charge_fill_eh_buf(l_di->dsm_buff,
			sizeof(l_di->dsm_buff),
...
int direct_charge_get_device_ibus(int *value)
{
	...
	if (dc_get_ic_ibus(l_di->cur_mode, value)) {
		direct_charge_fill_eh_buf(l_di->dsm_buff,
			sizeof(l_di->dsm_buff),
...
int direct_charge_init_adapter_and_device(void)
{
	...
	ret = dc_batinfo_init(l_di->cur_mode);
	if (ret) {
...
static int direct_charge_select_adp_max_curr(struct direct_charge_device *di)
{
	...
	adp_max_cur = direct_charge_get_adpter_max_current_by_adpter_type(
		di->adaptor_vset);
...
static int direct_charge_get_vstep(struct direct_charge_device *di)
{
	...
	if (dc_get_bat_current(di->cur_mode, &ibat))
		return di->vstep;
	...
	int enable)
{
	...
	if (enable && dc_batinfo_init(mode)) {
		di->multi_ic_error_cnt++;
		return -1;
	}
...
static void direct_charge_update_multi_info(struct direct_charge_device *di)
{
	...
	if (direct_charge_get_bat_current(&ibat) ||
		dc_get_ic_ibus(CHARGE_IC_MAIN, &ibus_main) ||
		dc_get_ic_ibus(CHARGE_IC_AUX, &ibus_aux))
		return;
	...
	for (i = 0; i < CHARGE_IC_MAX_NUM; i++) {
		...
		dc_get_ic_vout(BIT(i), &di->curr_info.vout[i]);
		di->curr_info.vbat[i] = dc_get_bat_btb_voltage_with_comp(BIT(i),
...
static void direct_charge_update_single_info(struct direct_charge_device *di)
{
	...
	dc_get_ic_ibus(CHARGE_IC_MAIN, &di->curr_info.ibus[0]);
	dc_get_ic_vout(CHARGE_IC_MAIN, &di->curr_info.vout[0]);
	di->curr_info.vbat[0] = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_MAIN,
		di->comp_para.vbat_comp);
}
...
static void direct_charge_get_ffc_para(struct direct_charge_device *di)
{
	...
	vbat_main = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_MAIN, di->comp_para.vbat_comp);
	vbat_aux = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_AUX, di->comp_para.vbat_comp);
...
void direct_charge_control_work(struct work_struct *work)
{
	...
	if (mulit_ic_check(di->cur_mode, &di->multi_ic_check_info, di->dc_volt_ratio) < 0) {
		di->multi_ic_error_cnt++;
		direct_charge_set_stop_charging_flag(1);
		direct_charge_stop_charging();
		return;
...
void direct_charge_stop_charging(void)
{
	...
	ret = dc_batinfo_exit(l_di->cur_mode);
	if (ret)
