...
int direct_charge_calculate_path_resistance_second(void)
{
	...
	for (i = 0; i < retry; ++i) {
		...
		hwlog_info("[%d]: Vapt=%d, Vbus=%d, Iapt=%d, Ibus=%d\n",
			i, tmp.vadapt, tmp.vbus, tmp.iadapt, tmp.vbus);
	}
	...
	snprintf(tmp_buf, sizeof(tmp_buf),
		"first Rpath=%d, Second Rpath=%d, Vapt=%d, Vbus=%d, Ibus=%d,\t"
		"Vapt0=%d, Vbus0=%d, Ibus0=%d\n",
...
int direct_charge_second_resist_handler(void)
{
	...
	for (i = 0; i < DC_RESIST_LEVEL; ++i) {
		if (path_resist >= l_di->second_resist_para[i].resist_min &&
			path_resist < l_di->second_resist_para[i].resist_max)
			return l_di->second_resist_para[i].resist_cur_max;
	}
