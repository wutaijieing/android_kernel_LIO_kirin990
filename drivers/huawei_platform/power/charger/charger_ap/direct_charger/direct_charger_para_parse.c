	...
	struct direct_charge_device *di)
{
	...
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"std_resist_para", idata, DC_RESIST_LEVEL, DC_RESIST_TOTAL);
	if (len < 0)
		return;

	for (row = 0; row < len / DC_RESIST_TOTAL; row++) {
		col = row * DC_RESIST_TOTAL + DC_RESIST_MIN;
		di->std_resist_para[row].resist_min = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_MAX;
		di->std_resist_para[row].resist_max = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_CUR_MAX;
		di->std_resist_para[row].resist_cur_max = idata[col];
	}
}

static void direct_charge_parse_second_resist_para(struct device_node *np,
	struct direct_charge_device *di)
{
	int row, col, len;
	int idata[DC_RESIST_LEVEL * DC_RESIST_TOTAL] = { 0 };
	...
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"second_resist_para", idata, DC_RESIST_LEVEL, DC_RESIST_TOTAL);
	if (len < 0)
		return;

	for (row = 0; row < len / DC_RESIST_TOTAL; row++) {
		col = row * DC_RESIST_TOTAL + DC_RESIST_MIN;
		di->second_resist_para[row].resist_min = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_MAX;
		di->second_resist_para[row].resist_max = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_CUR_MAX;
		di->second_resist_para[row].resist_cur_max = idata[col];
	}
}
	...
	struct direct_charge_device *di)
{
	...
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"resist_para", idata, DC_RESIST_LEVEL, DC_RESIST_TOTAL);
	if (len < 0)
		return;

	for (row = 0; row < len / DC_RESIST_TOTAL; row++) {
		col = row * DC_RESIST_TOTAL + DC_RESIST_MIN;
		di->nonstd_resist_para[row].resist_min = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_MAX;
		di->nonstd_resist_para[row].resist_max = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_CUR_MAX;
		di->nonstd_resist_para[row].resist_cur_max = idata[col];
	}
}
	...
	struct direct_charge_device *di)
{
	...
	len = power_dts_read_string_array(power_dts_tag(HWLOG_TAG), np,
		"ctc_resist_para", idata, DC_RESIST_LEVEL, DC_RESIST_TOTAL);
	if (len < 0)
		return;

	for (row = 0; row < len / DC_RESIST_TOTAL; row++) {
		col = row * DC_RESIST_TOTAL + DC_RESIST_MIN;
		di->ctc_resist_para[row].resist_min = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_MAX;
		di->ctc_resist_para[row].resist_max = idata[col];
		col = row * DC_RESIST_TOTAL + DC_RESIST_CUR_MAX;
		di->ctc_resist_para[row].resist_cur_max = idata[col];
	}
}
...
int direct_charge_parse_dts(struct device_node *np, void *p)
{
	...
	direct_charge_parse_second_resist_para(np, di);
