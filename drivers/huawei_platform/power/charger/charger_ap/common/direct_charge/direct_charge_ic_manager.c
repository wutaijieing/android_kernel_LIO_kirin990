...
static struct direct_charge_batinfo_ops *g_batinfo_ops[CHARGE_IC_MAX_NUM];
...
static inline struct direct_charge_batinfo_ops *dc_get_battinfo_ops(
	unsigned int type)
{
	if (type >= CHARGE_IC_TYPE_MAX)
		return NULL;

	return g_batinfo_ops[type];
}
...
int dc_batinfo_init(int type)
{
	...
	if (type & CHARGE_IC_MAIN) {
		temp_ops = dc_get_battinfo_ops(CHARGE_IC_TYPE_MAIN);
		if (temp_ops && temp_ops->init)
			ret = temp_ops->init();
	}
	if (type & CHARGE_IC_AUX) {
		temp_ops = dc_get_battinfo_ops(CHARGE_IC_TYPE_AUX);
		if (temp_ops && temp_ops->init)
			ret |= temp_ops->init();
	}
...
int dc_get_bat_btb_voltage_with_comp(int type, const int *vbat_comp)
{
	...
	vbat = dc_get_bat_btb_voltage(type);
	if (vbat < 0)
		return -1;
	else if (vbat == 0)
		return vbat;
...
int dc_get_bat_current_with_comp(int type, int *ibat)
{
	...
	if (dc_get_bat_current(type, ibat))
		return -1;
...
int dc_get_ic_max_temp(int *temp)
{
	...
	ret1 = dc_get_ic_temp(CHARGE_IC_MAIN, &temp1);
	ret2 = dc_get_ic_temp(CHARGE_IC_AUX, &temp2);
...
int dc_get_total_ibus(int *ibus)
{
	...
	if (!dc_get_ic_ibus(CHARGE_IC_MAIN, &main_ibus))
		ic_count++;
	if (!dc_get_ic_ibus(CHARGE_IC_AUX, &aux_ibus))
		ic_count++;
...
int dc_get_total_bat_current(int *ibat)
{
	...
	if (!dc_get_bat_current(CHARGE_IC_MAIN, &main_ibat))
		ic_count++;
	if (!dc_get_bat_current(CHARGE_IC_AUX, &aux_ibat))
		ic_count++;
...
int dc_get_bat_btb_avg_voltage(int *vbat)
{
	...
	main_vbat = dc_get_bat_btb_voltage(CHARGE_IC_MAIN);
	if (main_vbat > 0)
		ic_count++;

	aux_vbat = dc_get_bat_btb_voltage(CHARGE_IC_AUX);
	if (aux_vbat > 0)
		ic_count++;
...
int dc_get_bat_package_avg_voltage(int *vbat)
{
	...
	main_vbat = dc_get_bat_package_voltage(CHARGE_IC_MAIN);
	if (main_vbat > 0)
		ic_count++;

	aux_vbat = dc_get_bat_package_voltage(CHARGE_IC_AUX);
	if (aux_vbat > 0)
		ic_count++;
...
int dc_get_vbus_avg_voltage(int *vbus)
{
	...
	if (!dc_get_vbus_voltage(CHARGE_IC_MAIN, &main_vbus))
		ic_count++;
	if (!dc_get_vbus_voltage(CHARGE_IC_AUX, &aux_vbus))
		ic_count++;
