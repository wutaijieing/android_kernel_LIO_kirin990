...
static void multi_ic_dsm_report_dmd(int type, int dmd_level,
	struct multi_ic_check_para *info, const char *buf, int buf_size)
{
	...
	(void)dc_get_ic_ibus(CHARGE_IC_MAIN, &ibus_main);
	(void)dc_get_ic_ibus(CHARGE_IC_AUX, &ibus_aux);
	...
	snprintf(tmp_buf, sizeof(tmp_buf),
		...
		dc_get_bat_btb_voltage_with_comp(CHARGE_IC_MAIN, info->vbat_comp),
		dc_get_bat_btb_voltage_with_comp(CHARGE_IC_AUX, info->vbat_comp),
...
static void multi_ic_check_info(struct multi_ic_check_para *info)
{
	int ibat = 0;
	int i;
	int ibus_main = 0;
	...
	(void)dc_get_bat_current(CHARGE_IC_MAIN, &ibat);
	...
	if (ibus_ratio <= info_para->current_ratio_max &&
		...
		multi_ic_dsm_report_dmd(MULTI_IC_ERROR_TYPE_INFO,
			MULTI_IC_DMD_LEVEL_INFO, info, tmp_buf, strlen(tmp_buf));
	}
}

static void multi_ic_check_ibus(struct multi_ic_check_para *info)
{
	int ibus_main = 0;
	int ibus_aux = 0;
	int ibus_ratio;
	...
	for (i = 0; i < MULTI_IC_CURR_RATIO_PARA_LEVEL; i++) {
		...
		if (info->ibus_error_num[i] == info->curr_ratio[i].error_cnt) {
			snprintf(tmp_buf, sizeof(tmp_buf),
				"Ibus_ratio = %d, Ibus1 = %dmA, Ibus2 = %dmA\n",
				ibus_ratio, ibus_main, ibus_aux);
			multi_ic_dsm_report_dmd(MULTI_IC_ERROR_TYPE_IBUS,
...
static void multi_ic_check_vbat(struct multi_ic_check_para *info)
{
	int vbat_main;
	int vbat_aux;
	int delta_volt;
	...
	vbat_main = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_MAIN, info->vbat_comp);
	vbat_aux = dc_get_bat_btb_voltage_with_comp(CHARGE_IC_AUX, info->vbat_comp);
	...
	for (i = 0; i < MULTI_IC_VBAT_ERROR_PARA_LEVEL; i++) {
		...
		if (info->vbat_error_num[i] == info->vbat_error[i].error_cnt) {
			snprintf(tmp_buf, sizeof(tmp_buf),
				...
				delta_volt, vbat_main, vbat_aux);
			multi_ic_dsm_report_dmd(MULTI_IC_ERROR_TYPE_VBAT,
...
static void multi_ic_check_tbat(struct multi_ic_check_para *info)
{
	int tbat_main = 0;
	int tbat_aux = 0;
	int delta_temp;
...
static bool multi_ic_check_disable_status( int volt_ratio)
{
	int ibus_main = 0;
	int ibus_aux = 0;
	int isys;
	...
	(void)dc_get_ic_ibus(CHARGE_IC_MAIN, &ibus_main);
	(void)dc_get_ic_ibus(CHARGE_IC_AUX, &ibus_aux);
	(void)dc_get_bat_current_with_comp(CHARGE_IC_MAIN, &ibat);
...
int mulit_ic_check(int mode, struct multi_ic_check_para *info, int volt_ratio)
{
	u32 cur_time = current_kernel_time().tv_sec;
	int vbat_main;
	int vbat_aux;
	...
	if (multi_ic_check_disable_status(volt_ratio))
		return 0;
	...
	multi_ic_check_info(info);
	multi_ic_check_ibus(info);
	multi_ic_check_vbat(info);
	multi_ic_check_tbat(info);
