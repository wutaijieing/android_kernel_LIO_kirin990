...
static void init_battery_temp(struct multi_batt_temp_info *di)
{
	int i;
	...
	for (i = 0; i < di->sensor_num; i++) {
		di->sensor_tab[i].raw =
			power_temp_get_average_value(di->sensor_tab[i].name);
		di->sensor_tab[i].temp =
			get_batt_temp_with_comp(di->sensor_tab[i].raw);
		hwlog_info("init temp i:%d,ntc:%d,temp:%d\n",
			i, di->sensor_tab[i].raw, di->sensor_tab[i].temp);
	}
