...
static void btb_ck_bat_volt(struct btb_check_para *data, unsigned int *result)
{
	...
	main_btb = dc_get_bat_btb_voltage(CHARGE_IC_MAIN);
	...
	if (data->is_multi_btb) {
		aux_btb = dc_get_bat_btb_voltage(CHARGE_IC_AUX);
...
static struct power_sysfs_attr_info btb_ck_sysfs_field_tbl[] = {
	power_sysfs_attr_ro(btb_ck, 0440, BTB_CK_SYSFS_VOLT_RESULT, volt_result),
	power_sysfs_attr_ro(btb_ck, 0440, BTB_CK_SYSFS_TEMP_RESULT, temp_result),
};
