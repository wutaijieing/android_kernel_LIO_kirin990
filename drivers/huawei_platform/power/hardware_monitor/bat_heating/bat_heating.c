...
static bool bat_heating_check_temp(struct bat_heating_dev *l_dev)
{
	...
	l_dev->rf_temp = power_temp_get_average_value(POWER_TEMP_RF_BOARD);
	l_dev->usb_temp /= POWER_TEMP_UNIT;
	l_dev->rf_temp /= POWER_TEMP_UNIT;
	huawei_battery_temp(BAT_TEMP_MIXED, &l_dev->bat_temp);

	if ((abs(l_dev->bat_temp - l_dev->usb_temp) >= BAT_HEATING_USB_TEMP_THLD) ||
		(abs(l_dev->bat_temp - l_dev->rf_temp) >= BAT_HEATING_RF_TEMP_THLD)) {
		hwlog_info("check: usb_temp=%d rf_temp=%d bat_temp=%d\n",
			l_dev->usb_temp, l_dev->rf_temp, l_dev->bat_temp);
		return true;
	}
...
static void bat_heating_report_dmd_work(struct work_struct *work)
{
	...
	snprintf(buf, POWER_DSM_BUF_SIZE_0128 - 1,
		"count=%d chg_type=%d brand=%s volt=%d soc=%d t_bat=%d t_usb=%d t_rf=%d\n",
		l_dev->count,
