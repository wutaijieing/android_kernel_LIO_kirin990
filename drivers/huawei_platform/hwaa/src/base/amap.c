	...
	u32 data_size, u64 map_key)
{
	...
	struct map_entry_t *cursor = NULL;
...
{
	s64 status_code;
	struct map_entry_t *new_entry = NULL;
	if (!map || !data)
...
{
	s32 i;
	struct map_entry_t *map_head = NULL;
	struct map_entry_t *map_next = NULL;
